    ...
    let show = true;
        ...
        <div class="mx-auto md:m-0 rounded-2xl shadow-lg relative hover:shadow-2xl transition duration-500 {colorDictionary[color].mediumBackground} md:w-96 h-64 w-72 ">
            <div class="h-1/5 w-full flex justify-between flex-grow">
                ...
                <div class="active:bg-red-500 rounded-full {colorDictionary[color].darkText} active:text-white" on:click={() => dispatch('delete')}>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-8 m-2 transform hover:scale-50 active:scale-105 transition duration-300 ease-in-out active:scale-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="{colorDictionary[color].lightBackground} h-4/5 w-full rounded-2xl flex justify-between items-center flex-wrap">
                        ...
                        {#each chunk(bookIds, 2) as bookChunk, chunkIndex (chunkIndex)}
                            <div class="flex flex-row justify-center md:justify-around w-full h-36">
                                {#each bookChunk as id (id)}
                                    <img class="shadow-md  hover:shadow-1xl transform hover:scale-105 m-3" src="https://books.google.com/books?id={id}&printsec=frontcover&img=1&zoom=5&edge=curl&source=gbs_api" />
                                {/each}
                            </div>
                        {/each}
                    </svelte:component>
                {:else}
                    <div class="flex flex-row h-full" on:click={() => (show = !show)}>
                        ...
                        <h1 class="font-extrabold text-2xl {colorDictionary[color].darkText} m-4">Add a book and let's get started!</h1>
                    </div>    
                {/if}    
            </div>
        </div>