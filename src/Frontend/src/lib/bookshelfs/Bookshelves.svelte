...
<div class="min-h-screen bg-bookpage p-4 w-full h-full">
    ...
    {#each bookshelves as bookshelf, index (bookshelf.id)}
        <div class="p-5" transition:fade={{duration: 250, easing: quintOut}} animate:flip="{{delay: 250, duration: 250, easing: quintOut}}">
            <Preview name={bookshelf.name} color={getColor(bookshelf.id)} categories="{bookshelf.categories}" bookIds="{bookshelf.bookIds}" on:delete={() => showModalAndSelectIndex(index)}/>
        </div>
    {/each}
    </div>
</div>
<div class="fixed bottom-4 right-4">
    <button class="inline-flex items-center justify-center md:h-20 md:w-20 h-12 w-12 mr-2 text-indigo-100 transition-colors duration-150 bg-indigo-700 rounded-lg focus:shadow-outline hover:bg-indigo-800" on:click="{() => nameModal = true}">
        <svg class="md:h-16 md:w-16 h-8 w-8 fill-current" viewBox="0 0 20 20"><path d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" fill-rule="evenodd"></path></svg>
    </button>
</div>
<DeleteBookshelfModal bind:show={deleteModal} on:continue={() => deleteBookshelf()} on:cancel={() => deleteModal = false}/>
<InputNameModal bind:show={nameModal} on:create={addNewBookshelf} on:cancel={() => nameModal = false}/>