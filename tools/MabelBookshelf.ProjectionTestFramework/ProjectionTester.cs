...
namespace MabelBookshelf.ProjectionTestFramework
{
    public class ProjectionTester
    {
        ...
        public async Task<List<(string domainEvent, int entityCount, TimeSpan time)>> TestProjection(IProjectionService service)
        {
            List<(string domainEvent, int entityCount, TimeSpan time)> result =
        ...
        private async Task<TimeSpan> Time(List<StreamEntry> events, IProjectionService service)
        {
            ...
            foreach (var @event in events)
            {
                await service.ProjectAsync(@event);
            }
            watch.Stop();
